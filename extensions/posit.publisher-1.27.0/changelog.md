...
- Fixed an issue where the preferred Python path was incorrectly not found when deploying [#2742](https://github.com/posit-dev/publisher/issues/2742)
- Fixed Python dependencies scanning not respecting the active Python session [#2825](https://github.com/posit-dev/publisher/issues/2825)
...
- Added a "Render Your Project" button that shows up for HTML project deployments rendered by Quarto
  to easily update and publish the rendered output [#2948](https://github.com/posit-dev/publisher/issues/2948)
...
- Fixed the Python and R Packages views incorrectly stating the default package
  files were missing even when they were present (#2882, [#2884](https://github.com/posit-dev/publisher/issues/2884))
...
- Added schema and agent support for publishing to Connect Cloud. (#2729, [#2747](https://github.com/posit-dev/publisher/issues/2747), [#2771](https://github.com/posit-dev/publisher/issues/2771))
...
- When opened, the Publisher logs scroll to and expand the first failed stage
  if a stage is marked as failed (#2857, [#2858](https://github.com/posit-dev/publisher/issues/2858))
...
- Improved support for Posit Connect deployments hosted in Snowpark Container
  Services. (#2687, [#2691](https://github.com/posit-dev/publisher/issues/2691))
...
- R, Python, and Quarto versions are now optional in configuration files and
  ...
  running environment. (#2468, [#2470](https://github.com/posit-dev/publisher/issues/2470))
...
- Added support for deploying Gradio content to Posit Connect. (#2476, [#2477](https://github.com/posit-dev/publisher/issues/2477))
...
- The Python interpreter used is now consistent across the extension and
  ...
  defaulting to the Python on PATH when none is selected. (#2494, [#2496](https://github.com/posit-dev/publisher/issues/2496))
...
- Deployment dismissal has been greatly improved. Deployments are no longer
  ...
  the dismissed state, avoiding changes after dismissal. (#2179, [#2498](https://github.com/posit-dev/publisher/issues/2498))
...
- Warnings in the sidebar have a new look to optimize horizontal space, utilize
  theme colors, and emphasize links. (#2526, [#2527](https://github.com/posit-dev/publisher/issues/2527))
...
- The sidebar design has changed slightly to emphasize the deployment selection.
  (#2480, [#2481](https://github.com/posit-dev/publisher/issues/2481))
...
- Selecting a configuration file with errors no longer disables the deploy
  ...
  indicate what needs to be addressed (#2345, [#2374](https://github.com/posit-dev/publisher/issues/2374))
...
- Added helpful errors when attempting to deploy with a missing package file, or
  a package file with no content (#2177, [#2327](https://github.com/posit-dev/publisher/issues/2327))
...
- Secrets are now able to be pushed up during a deploy in the added Secrets
  ...
  individually in the Secrets view. (#2260, [#2304](https://github.com/posit-dev/publisher/issues/2304), [#2305](https://github.com/posit-dev/publisher/issues/2305))
...
- Removed the flat file view for Project Files in favor of the tree view
  (#1210, [#2160](https://github.com/posit-dev/publisher/issues/2160))
...
- Added inspection support for Jupyter notebooks using Quarto Inspect allowing
  Jupyter notebooks to be published using Quarto (#2059, [#2085](https://github.com/posit-dev/publisher/issues/2085))
